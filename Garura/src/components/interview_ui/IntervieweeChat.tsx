  ...
  return (
    <div className="h-full flex flex-col bg-slate-800/50 rounded-lg border border-slate-700 overflow-hidden">
          ...
          messages.map((msg) => (
            <div
              ...
              className={`flex w-full ${
                msg.sender === 'interviewee' ? 'justify-end' : 'justify-start'
              }`}
            >
              <div
                className={`max-w-[85%] rounded-lg px-3 py-2 text-white break-words ${
                  msg.sender === 'interviewee' ? 'bg-blue-600' :
                  msg.sender === 'System' ? 'bg-yellow-800/60' :
                  'bg-slate-700'
                }`}
              >
      ...
      <form onSubmit={handleSend} className="p-2 border-t border-slate-700 flex gap-2">
        ...
        <button
          type="submit"
          className="w-10 h-10 flex items-center justify-center bg-blue-600 hover:bg-blue-700 rounded font-semibold shrink-0"
        >
          <Send size={18} />
        </button>
      </form>
